...
#define OV534_REG_SCCB_ADDRESS   0xf1   /* ? */
...
{	
    ...
    if (!isGrabbing) 
    {
		...
		if (resolution == ResolutionVGA)
        {
            ...
            [self setRegister:0x1d toValue:0x02];
            [self setRegister:0x1d toValue:0x58];
            [self setRegister:0x1d toValue:0x00];
            [self setRegister:0xc0 toValue:0x50];
            [self setRegister:0xc1 toValue:0x3c];
            
            [self setSensorRegister:0x12 toValue:0x00];
...
//          [self setSensorRegister:0x0c toValue:0xd0]; // flip
            ...
            else if (fps == 60 || fps == 0) // previously 60 // measured 100 but image not moving
            {
				[self setSensorRegister:0x11 toValue:0x01];
				[self setSensorRegister:0x0d toValue:0xc1];
				[self verifySetRegister:0xe5 toValue:0x04];
        ...
        else if (resolution == ResolutionSIF) 
        {
            ...
            [self setRegister:0x1d toValue:0x02];
            [self setRegister:0x1d toValue:0x00];
            [self setRegister:0x1d toValue:0x01];
            [self setRegister:0x1d toValue:0x4b];
            [self setRegister:0x1d toValue:0x00];
            [self setRegister:0xc0 toValue:0x28];
            [self setRegister:0xc1 toValue:0x1e];
            
            [self setSensorRegister:0x12 toValue:0x40];
            [self setSensorRegister:0x17 toValue:0x3f];
            [self setSensorRegister:0x18 toValue:0x50];
            [self setSensorRegister:0x19 toValue:0x03];
            [self setSensorRegister:0x1a toValue:0x78];
            [self setSensorRegister:0x29 toValue:0x50];
...
//          [self setSensorRegister:0x0c toValue:0xd0]; // flip
			...
			else if (fps == 125 || fps == 0) // previously 125 // measured 140 (received 230!?), some artifacts
            {
				[self setSensorRegister:0x11 toValue:0x02];
				[self setSensorRegister:0x0d toValue:0x81];
				[self verifySetRegister:0xe5 toValue:0x02];
...
//  [self setRegister:0xf1 toValue:0x60]; // for other sensors?
    [self setRegister:0xf1 toValue:0x42]; // OV722x sensor
    ...
    [self setRegister:0x1c toValue:0x00];
    [self setRegister:0x1d toValue:0x40];
    [self setRegister:0x1d toValue:0x02]; // payload size 0x0200 * 4 = 2048 bytes
    [self setRegister:0x1d toValue:0x00]; // payload size
    
    [self setRegister:0x1d toValue:0x02]; // frame size // original
    [self setRegister:0x1d toValue:0x57]; // frame size
    [self setRegister:0x1d toValue:0xff]; // frame size
    /*
    [self setRegister:0x1d toValue:0x01]; // frame size
    [self setRegister:0x1d toValue:0x4b]; // frame size
    [self setRegister:0x1d toValue:0x00]; // frame size
     */
    /*
    [self setRegister:0x1c toValue:0x0a];
    [self setRegister:0x1d toValue:0x08]; // turn on UVC header
    [self setRegister:0x1d toValue:0x0e];
    */
    ...
    printf("sensor is OV%02x%02x", [self getSensorRegister:0x0a], [self getSensorRegister:0x0b]);
    ...
    [self setSensorRegister:0x13 toValue:0xf7]; // AWB?
    [self setSensorRegister:0x0d toValue:0x41];
    ...
    [self setSensorRegister:0x13 toValue:0xff]; // AWB?
...
//*/    
    ...
    [self setSensorRegister:0x8e toValue:0x00]; // 0x04 according to latest
    [self setSensorRegister:0x0c toValue:0xd0];
